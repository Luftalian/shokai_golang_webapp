...
func AssertResponse(t *testing.T, got *http.Response, status int, body []byte) {
	...
	if got.StatusCode != status {
		t.Fatalf("want status %d, but got %d b, body: %q", status,got.StatusCode, gb)
	}

	if len(gb) == 0 && len(body) == 0 {
		// Because both the expected body and the actual body are empty, we don't need to call AssertJSON.
		return
	}
